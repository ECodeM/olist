...
class Seller:
    ...
    def get_active_dates(self):
        ...
        orders_approved = self.data['orders'][['orders_id', 'order_approved_at']].dropna()

        # Then, create a (orders <> sellers) join table because a seller can appear multiple times in the same order

        orders_sellers = orders_approved.merge(self.data['orders'],
                                               on='order_id')[[
                                                   'order_id',
                                                   'seller_id',
                                                   'order_approved_at']]

        orders_sellers['orders_approved_at'] = pd.to_datetime(orders_sellers['order_approved_at'])
